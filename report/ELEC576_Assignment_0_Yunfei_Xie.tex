...
the alternative commands \cite{numpy}. Each entry follows the table's
row order and includes its code and captured output. Array entries are
displayed with up to six decimal places; calculations use full
floating-point precision.

The random-number and resampling entries show the errors from the
table's calls, followed by working calls. Notes explain differences in
...
The table's ix\_ call is missing a closing parenthesis. The example includes it.
...
\solution
I used broadcasting to compute pairwise distances between $(0,0)$,
$(3,4)$, and $(0,4)$, then practiced reductions, Boolean indexing,
and copying \cite{stanford}. The three distances are $5$, $4$, and $3$.
Changing a slice changes the original array; changing a copy leaves
...
\solution
The line passes through $(1,1)$, $(2,2)$, $(3,7)$, and $(4,14)$. The horizontal limits are
...
\end{center}
The repository contains the report, executed notebook, and Python
source files with their figures and results. The README gives the
commands to run the code and numerical checks.
